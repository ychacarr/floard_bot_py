...
#     async with ClientSession(headers=session_header) as session:
...
async def congrats_from_yandex(name: str) -> str:
    ...
    """
    congrats_text = f'{name}' + '! Поздравляю тебя с днём рождения! Желаю, как и всегда, здоровья, счастья и '
    post_payload = {"query": congrats_text, "intro": 0, "filter": 1}
    ...
    async with ClientSession() as session:
        async with session.post('https://yandex.ru/lab/api/yalm/text3', json= post_payload) as resp:
            if resp.status == 200:
                ...
                json_data = await resp.json()
                balaboba_congrats = json_data["text"]
                congrats_text += balaboba_congrats
                return congrats_text
            else:
                raise HTTPException(status= resp.status, reason= "Connection to Balaboba failed!")   
...
async def create_congrats(member_name: str) -> str:
    ...
    try:
        congrats = await congrats_from_yandex(member_name)
    except HTTPException as ex:
        log.error(f"{ex.reason} Response status: {ex.status}")
    finally:
        return congrats
