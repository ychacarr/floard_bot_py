...
class Job:
    ...
    Использование:
        ...
        пометить работу как незаписываемую в backup файл.
    ...
    def next_datetime(self) -> bool:
        """
        На основе period_name и period_amount вычисляет сладующие дату и время выполнения работы.
...
class AsyncScheduler:
    ...
    Содержит список работ. Уникальность работы в списке определяется имнем. Существование двух работ с одинаковыми
    ...
    реализовано в виде добавления новых задач (task) в запущенный цикл asyncio. Все задачи будут гарантированно
    добавлены в цикл, будут ли они все выполнены - зависит от состояния цикла и работы asyncio.\n Ожидание до времени
    срабатывания следующей работы выполнено через asyncio.sleep. Планировщик не имеет жестко заданного интервала
    ожидания, вместо этого время ожидания рассчитывается на основе вычитания:\n время выполнения следующей задачи -
    текущее время\n Вычитание дат и времени реализовано через стандартный модуль datetime. Текущее время получается с
    помощью datetime.now()\n Если в момент ожидания следующей работы в расписание добавляется новая работа,
    планировщик прерывает сон, добавляет работу и перезапускает цикл ожидания.\n
    ...
    Использование:\n
        ...
        Пример:\n
            async main():\n
                ...
                (т.е. когда очередь задач будет пуста, если в расписании есть хотя бы одна периодиеская задача,
        ...
        (т.е. только после вызова asyncio.run_untill_complete() или asyncio.run_forever()).
