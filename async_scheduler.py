...
class Job:
    """
    Класс инкапсулирующий некоторую периодическую|разовую работу.
    ...
    Использование:
        Для создания разовой работы (т.е. которая будет выполнена единожды в 10.08.22 12:00) с некоторой функцией
        some_func и параметрами:\n
        ...
        Job(
            ...
            {'arg1': 1, 'arg2': 'something', 'arg3': 2.3},
            Periods.once,
            str_datetime='10.08.22 12:00'
        )

        Для создания периодической работы с периодом выполнения каждые 3 минуты (начиная с 10.08.22 10:00) с функцией
        my_func и параметрами:\n
            testing = 3\n
        необходимо вызвать следующий конструктор:\n
        Job('second_job', my_func, {'testing': 3}, Periods.minute, 3, '10.08.22 10:00')

        Для создания периодической работы с периодом выполнения каждый год (начиная с 10.08.22 10:00) с функцией f без
        параметров необходимо вызвать следующий конструктор:\n
        Job('third_job', f, period_name= Periods.year, period_amount= 1, str_datetime= '10.08.22 10:00')\n

        В объекте работы есть свойство need_backup, которое определяет нужно ли записывать эту работу в backup файл
        AsyncSchedule. Если задача создаётся динамически (т.е. при возникновении некоторого события, которое
        определяется только во время работы программы), то это свойство следует задать как True  (по умолчанию), в
        противном случае (если работа создаётся и заносится в расписание каждый раз при старте программы) следует
        пометить работу как не записываемую в backup файл.
    Объект класса Job не отслеживает текущее время и не выполняется сам по себе! Для добавления работы в очередь
    выполнения используйте класс AsyncScheduler.
    """
    ...
    def pack_to_str(self) -> str:
        ...
        """
        return f"{self._name}|{self._func.__name__}|{self._func.__module__}|{self._func_kwargs}|{self._period_name}|" \
               f"{self._period_amount}|{self.job_datetime_str}"
    ...
    def next_datetime(self) -> bool:
        """
        На основе period_name и period_amount вычисляет следующие дату и время выполнения работы.
        Записывает новые значения в объект.
    ...
    def period_name_set(self, new_period: str):
        ...
        """
        if new_period == 'once' or new_period is None:
            self._period_name = 'once'
        elif new_period in {'minute', 'hour', 'day', 'week', 'month', 'year'}:
            if self._period_amount <= 0:
                raise ValueError("async_scheduler.Job.period_name_set() error."
                                 "period_amount can be zero only when period_name = 'once'.")
            else:
    ...
    def period_amount_set(self, new_amount: int):
        ...
        elif new_amount == 0 and (self._period_name != 'once') and (self._period_name is not None):
            raise ValueError("async_scheduler.Job.period_amount_set() error."
                             "period_amount can be zero only when period_name = once.")
        else:
...
class AsyncScheduler:
    ...
    Содержит список работ. Уникальность работы в списке определяется именем. Существование двух работ с одинаковыми
    именами не допускается. (При добавлении новой работы, если в расписании уже существует работа с таким именем,
    новая игнорируется).\n

    Добавление работы с "просроченным" сроком выполнения невозможно. (будет выброшено исключение)\n
    Выполнение работ реализовано в виде добавления новых задач (task) в запущенный цикл asyncio.
    Все задачи будут гарантированно добавлены в цикл, будут ли они все выполнены - зависит от состояния цикла и
    работы asyncio.\n
    Ожидание до времени срабатывания следующей работы выполнено через asyncio.sleep. Планировщик не имеет жестко
    заданного интервала ожидания, вместо этого время ожидания рассчитывается на основе вычитания:\n
        время выполнения следующей задачи - текущее время\n
    Вычитание дат и времени реализовано через стандартный модуль datetime. Текущее время получается с
    помощью datetime.now()\n
    Если в момент ожидания следующей работы в расписание добавляется новая работа, планировщик прерывает сон,
    добавляет работу и перезапускает цикл ожидания.\n

    Реализована функция записи невыполненных работ в файл с возможностью их последующего восстановления. (может
    пригодиться в случае аварийного завершения программы)
    
    Использование:\n
        Класс предусматривает два варианта запуска планировщика:\n
            Вызов .run()\n
            Вызов .add_to_loop()\n
        Вызов .run() запускает цикл ожидания прямо внутри себя. Дальнейший код (расположенный за вызовом .run будет
        выполнен только после того как цикл завершится).
        Использовать .run стоит в том случае, если планировщик является основной задачей (task) всего приложения.
        Пример:\n
            async main():\n
                scheduler = AsyncScheduler(someList)\n
                await scheduler.run()\n
                ...код здесь будет выполнен только после завершения работы цикла планировщика
                (т.е. когда очередь задач будет пуста, если в расписании есть хотя бы одна периодическая задача,
                цикл остановится только через системное прерывание)\n
        Вызов .add_to_loop() создает в существующем цикле новую asyncio задачу с функцией self.run().
        После срабатывания, интерпретатор продолжит выполнять инструкции расположенные за вызовом .add_to_loop().
        Планировщик запустится только после запуска основного цикла событий
        (т.е. только после вызова asyncio.run_until_complete() или asyncio.run_forever()).
        Если код после вызова .add_to_loop() не запускает цикл, планировщик не будет запущен.
        Пример:\n
            async main():\n
                scheduler = AsyncScheduler(someList)\n
                ...
                loop.run_until_complete() - цикл планировщика задач начнет выполнение с этого момента\n
        Возможна ситуация, когда некоторая задача, запускающая цикл событий, перехватывает все исключения и прерывания,
        не давая планировщику отработать функцию записи невыполненных работ в файл
        (пример: использование планировщика вместе с ботом aiogram).
        В таком случае в main есть смысл добавить строку вида:\n
            scheduler.do_backup()\n
        Чтобы функция точно сработала её можно добавить в блок finally. Пример:\n
            ...код main...\n
            try:\n
    ...
    def __init__(self, jobs: List[Job] = None, backup_flag: bool = False, backup_filename: str = None):
        ...
        if backup_flag:
            if backup_filename is not None:
                ...
                raise ValueError("async_scheduler.AsyncScheduler.__init__ error."
                                 "backup_filename can't be None when backup_flag = True.")
    ...
    def do_backup(self):
        ...
        if self._backup_file is not None:
            ...
            raise ValueError("async_scheduler.AsyncScheduler._do_backup error."
                             "Missing backup filename. self._backup_file = None.")
    ...
    async def run(self) -> None:
        ...
        Во всех случаях, кроме последнего, планировщик остановится, выполнит вызовет .do_backup(), если это необходимо,
        и завершит работу.\n
        ...
        try:
            if self._loop_task is not None:
                ...
                while self._loop_task is not None:
                ...
                if len(self._jobs) > 0:
                    ...
                    if now_datetime == self._jobs[0]:
                        ...
                        if now_datetime > self._jobs[0]:
                            ...
                            # используется datetime.datetime.now() так как в now_datetime отброшены секунды и
                            # микросекунды, что приводит к неправильным расчётам т.е. xx:05:00 - yy:04:00 = zz:01:00
                            # - ожидание в минуту, несмотря на то, что текущее время может быть yy:04:40 и ждать
                            # нужно лишь оставшиеся 20 секунд
                            now_time = datetime.datetime.now().replace(microsecond=0)
                            time_to_wait = (self._jobs[0].job_datetime - now_time).seconds
            ...
            self._loop_task = None
            if not self._stopped:
                if self._backup_file is not None and len(self._jobs) != 0:
    ...
    def _search_name(self, name: str) -> int | None:
        ...
        name -- строка, содержащая имя работы.\n
    ...
    def add_job(self, job: Job) -> bool:
        ...
        Возвращает:\n
            ...
            False - в ином случае.\n
        Если на момент добавления работы, цикл планировщика уже запущен, он будет перезапущен посредством вызова .stop
        и последующего вызова .add_to_loop
        """
        if isinstance(job, Job):
            if self._search_name(job.name) is None:
                now_time = datetime.datetime.now().replace(second=0, microsecond=0)
                if now_time > job.job_datetime:
